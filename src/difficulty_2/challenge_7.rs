...
fn main() {
    let distance: f64 = input("Enter the distance (in metres): ").parse::<f64>().expect("Please enter a valid number");
    let time: f64 = input("Enter the time (in seconds): ").parse::<f64>().expect("Please enter a valid number");
